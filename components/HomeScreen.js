...
import { ListItem, Icon, Overlay, Button, Input, Text, Image, FAB } from 'react-native-elements';
...
const HomeScreen = ({ navigation }) => {
	...
	useEffect(() => {
		firestore()
			...
			.doc(userID)
			.update({ trucks: truckList })
			.then(() => {
				console.log('truckList updated!');
			})
			.catch((err) => {
				console.log(err);
			});
		...
		firestore()
			...
			.then((snapshot) => {
				if (snapshot.exists) {
					...
					setHowManyScalesLeft(userData.howManyScalesLeft ? userData.howManyScalesLeft : 0);
					setTruckList(userData.trucks ? userData.trucks : {});
		...
		<SafeAreaProvider>
			...
			<Overlay isVisible={overlayVisible} onBackdropPress={Keyboard.dismiss}>
				<View style={styles.loginOverlay}>
					<Image source={require('./Images/truck.png')} containerStyle={{ width: 100, height: 100 }} />
					<Text style={styles.loginOverlayTextWelcome}>ברוך הבא!</Text>
					<Text style={styles.loginOverlayTextPlease}>אנא הכנס את קוד המשתמש:</Text>
					<Input
					...
					/>
					<Button buttonStyle={styles.loginButton} title='התחבר' onPress={performLoginWithUserId} loading={isLoading} />
					{isUserFound ? null : <Text style={styles.userNorFoundText}>המשתמש לא קיים</Text>}
				</View>
			</Overlay>
			<View style={styles.container}>
				...
				<Text style={styles.howManyleftText}>{`נשארו לך עוד ${howManyScalesLeft} שקילות`}</Text>
				<View style={styles.pdfScrollView}>
					<ScrollView>
						{pdfInvoices
							? pdfInvoices.map((l, i) => (
									<ListItem key={i} bottomDivider='bottomDivider' onPress={() => navigation.navigate('InvoiceViewer', { uri: l.uri })}>
										<Icon name={'file-copy'} />
										<ListItem.Content>
				...
				<FAB style={styles.shareButton} color={'#722ed1'} icon={<Icon name='truck' color='white' type='material-community' />} onPress={() => navigation.navigate('TrucksList', { truckList: truckList, updateTruckList: setTruckList })} />
			</View>
		</SafeAreaProvider>
	);
};
