...
const InvoiceGenerator = ({route, navigation}) => {
  ...
  const generateInvoiceWithParameters = () => {
    ...
    newInvoice = newInvoice.replace(
      constants.SCALER_NAME,
      scalerName.toString(),
    );
    newInvoice = newInvoice.replace(
      constants.SCALER_PHONE,
      scalerPhone.toString(),
    );
    ...
    newInvoice = newInvoice.replace(
      constants.TOTAL_WEIGHT,
      weightReading.toString(),
    );
  ...
  const createInvoiceAsPDFAndUpload = async newInvoice => {
    ...
    const reference = storage().ref(
      '/PDFInvoices/' + userId + '/' + invoiceSubTitle + '.pdf',
    );
